				...
				<div class="collapse navbar-collapse" id="navbarSupportedContent">
					<ul class="navbar-nav me-auto mb-2 mb-lg-0">
						...
						<sec:authorize access="isAuthenticated()">
							...
							<c:choose>
								<c:when
									test="${auths.stream().anyMatch(authVO -> authVO.auth.equals('ROLE_ADMIM')).get() }">
									<!-- admin 계정만 처리 가능 -->
									<li class="nav-item"><a class="nav-link"
										href="/member/list">크하하${authNickName}${authEmail}</a></li>
								</c:when>
								<c:otherwise>
									<li class="nav-item"><a class="nav-link"
										href="/member/detail?email=${authEmail }">${authNickName}${authEmail}</a>
									</li>
							</c:otherwise>
						</c:choose>
									<!--로그인 해야 open -->
									...
									<li class="nav-item"><a class="nav-link"
										href="/member/logout" id="logOutLink">LogOut</a></li>
									<form action="/member/logout" method="post" id="logoutForm">
										<input type="hidden" name="email" value="${authEmail}">
									</form>
									<li class="nav-item"><a class="nav-link"
										href="/member/detail">Member Detail</a></li>
						...
						<li class="nav-item"><a class="nav-link"
							href="/member/register">SingUp</a></li>
							</sec:authorize>
						<!-- 로그아웃  끝 -->
